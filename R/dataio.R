...
## Date:   2017-04-28
##
## R code to download and read the 2015 ONS baby names data from the
## ONS web site. The ONS data only contains names with more than two
## uses in the year. As a consequence, an inquiry with the ONS
## customer support was made to get the number of names occurring once
## and twice for girls and boys. We add these as artificial names to
## the regular ONS dataset.
######################################################################
...
newborn_not12 <- bind_rows(
  lapply( c(girls="girls",boys="boys"), function(sex) {
    ##Load CSV file
    destfile <- file.path("..","Data",paste0("2015",sex,"namesfinal.xls"))
    ...
    df <- read_excel(path=destfile,sheet="Table 6",skip=4)
...
save(file=file.path("..","Data","newborn.RData"),list=c("newborn"))
